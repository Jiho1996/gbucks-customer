  ...
  template() {
    return `
        <div class = "slider-title-explain-container">
            <p id = "${
              this.category
            }-title" class ="title-class"}>${this.category.toUpperCase()}</p>
            <p id ="${this.category}-explain" class ="explain-class"}>${
      explanation.menuExplain[this.category]
    }</p>
        </div>
            <ul id ="${this.category}-slides_list" class="slides_list"></ul>

            <div id ="${this.category}-slide_btn_box" class="slide_btn_box">
              <button type="button"><img src = "../img/left.png" id="${
                this.category
              }-slide_btn_prev" class="slide_btn_prev"></button>
                    <button type="button"><img src = "../img/right.png" id="${
                      this.category
                    }-slide_btn_next" class="slide_btn_next"></button>
                        </div>
                    `;
  }

  mount() {
    new MenuSliderElement(
      $(`#${this.category}-slides_list`),
      this.category,
      this.$state
    );
  }
